...
export default class Crontab extends React.Component {
  constructor(props) {
    super(props);
    this.state =
      props.initialValue && props.initialValue.crontabUi
        ? JSON.parse(props.initialValue.crontabUi)
        : data;
    ...
    return (
      <div>
        <RadioGroup defaultValue={minute.type} onChange={this.onMinTypeChange}>
          ...
          <Radio style={radioStyle} value='define'>
            指定&nbsp;&nbsp;
            <Select
              mode='multiple'
              style={{ width: 280 }}
  ...
  dayContent = day => {
    ...
    return (
      <div>
        ...
        >
          <Radio style={radioStyle} value='unused'>
            不使用天&nbsp;&nbsp;
          </Radio>
          <Radio style={radioStyle} value='loop'>
            每天&nbsp;&nbsp;
  ...
  weeklyContent = weekly => {
    ...
    return (
      <div>
        <RadioGroup
          defaultValue={weekly.type}
          value={weekly.type}
          onChange={this.onWeeklyTypeChange}
        >
          <Radio style={radioStyle} value='unused'>
            不使用星期&nbsp;&nbsp;
          </Radio>
          ...
          <Radio style={radioStyle} value='define'>
            ...
            >
              <Option key='mon'>星期一</Option>
              <Option key='tue'>星期二</Option>
              <Option key='wed'>星期三</Option>
              <Option key='thu'>星期四</Option>
              <Option key='fri'>星期五</Option>
              <Option key='sat'>星期六</Option>
              <Option key='sun'>星期日</Option>
            </Select>
          </Radio>
        </RadioGroup>
      </div>
  ...
  getCrontab = data => {
    ...
    if (data.type === 'define') {
      ...
      if (data.minute.type === 'loop') {
        ...
        crontabArr[1] = data.minute.data.index.length
          ? data.minute.data.index.join(',')
          : '*';
      }
      ...
      if (data.hour.type === 'loop') {
        ...
        crontabArr[2] = data.hour.data.index.length
          ? data.hour.data.index.join(',')
          : '*';
      }
      ...
      if (data.day.type === 'loop') {
        ...
        crontabArr[3] = data.day.data.index.length
          ? data.day.data.index.join(',')
          : '*';
      } else {
        crontabArr[3] = '?';
      }
      ...
      if (data.month.type === 'loop') {
        ...
        crontabArr[4] = data.month.data.index.length
          ? data.month.data.index.join(',')
          : '*';
      }
      ...
      if (data.weekly.type === 'loop') {
        ...
        crontabArr[5] = data.weekly.data.index.length
          ? data.weekly.data.index.join(',')
          : '*';
      } else {
        crontabArr[5] = '?';
      }
  ...
  render() {
    ...
    const type = getFieldValue('type') || 'define';
    return (
      <div>
          ...
          <FormItem {...tableFormItemLayout} label=''>
            {getFieldDecorator(
              ...
              <div className={styles.cardContainer}>
                ...
                <div className={styles['crontab-input']}>
                  <Input
                    disabled={true}
                    style={{
                      float: 'left',
                    ...
                    defaultValue={data.crontab}
                    value={this.state.crontabExpression}
                  />
                </div>
              </div>
            )}
