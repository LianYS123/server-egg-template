...
export function createTableAsyncAction(options) {
  ...
  async function searchTable(state, action, dispatch, select, url) {
    const searchPayload = cloneDeep(action.payload);
    if (searchPayload.cb) {
      delete searchPayload.cb;
    }
    try {
      dispatch([
        {
   ...
   * 获取数据带参数
   */
  async function getTableDataWithArgs(state, action, dispatch, select, url) {
    try {
      dispatch([
        {
          type: `${actionNamePrefix}/set-query`,
  ...
  return {
    ...
    [`${actionNamePrefix}/getWithArgs`]: getTableDataWithArgs
  };
}
